/**
 * @fileoverview Comprehensive final test suite for portfolio functionality
 * Tests end-to-end flows, performance, security, storage backends, image processing, and limits
 * This is the complete implementation of task 11 requirements
 */
...
describe('Portfolio Comprehensive Final Test Suite', () => {
  let providerToken;
  let clientToken;
  let providerId;
  let clientId;
  ...
  beforeAll(async () => {
    ...
    // Create another provider for cross-access tests
    ...
    otherProviderToken = generateToken({
      sub: otherProvider.id,
      language: otherProvider.language || 'en',
  ...
  afterAll(async () => {
    // Clean up test data
    const userIds = [providerId, clientId, otherProviderId].filter(Boolean);
    await ProviderPortfolio.destroy({ where: { provider_id: userIds } });
  ...
  beforeEach(async () => {
    ...
    const userIds = [providerId, otherProviderId].filter(Boolean);
    await ProviderPortfolio.destroy({ where: { provider_id: userIds } });
    
    // Reset config to defaults with correct structure
    config.portfolio = {
      storage: {
        type: 'local',
        local: {
  ...
  describe('End-to-End Portfolio Flow Tests', () => {
    ...
    test('should handle portfolio with maximum allowed images', async () => {
      ...
      config.portfolio.limits.maxImages = 3;
    ...
    test('should handle empty portfolio gracefully', async () => {
      ...
      expect(statsResponse.body.data.remainingSlots).toBe(config.portfolio.limits.maxImages);
    });
  });

  describe('Performance Tests for Concurrent Operations', () => {
    ...
    test('should handle large file uploads within limits', async () => {
      ...
      const largeImage = await createTestImage(1800, 1200, 'jpeg', 0.8); // High quality
  ...
  describe('Security Tests for File Validation and Access Controls', () => {
    test('should reject malicious file types', async () => {
      const maliciousFiles = [
        { content: 'MZ', name: 'malware.jpg' },
        { content: 'ELF', name: 'malware.png' },
        { content: '#!/bin/bash', name: 'script.jpeg' },
        { content: '<?php system($_GET["cmd"]); ?>', name: 'webshell.jpg' }
      ];
      ...
      for (const file of maliciousFiles) {
        ...
        const response = await request(app)
          .post('/api/users/profile/portfolio')
          .set('Authorization', `Bearer ${providerToken}`)
          .attach('image', maliciousBuffer, file.name);

        expect(response.status).toBe(400);
        expect(response.body.error).toMatch(/file type|invalid|image/i);
      }
    });
    ...
    test('should prevent cross-provider portfolio access', async () => {
      ...
      const uploadResponse = await request(app)
        .post('/api/users/profile/portfolio')
        .set('Authorization', `Bearer ${providerToken}`)
        .attach('image', testImage, 'cross-access-test.jpg');
  ...
  describe('Storage Backend Switching and Configuration Tests', () => {
    ...
    test('should handle storage failures gracefully', async () => {
      ...
      const originalPath = config.portfolio.storage.local.path;
      ...
      // Restore config
      config.portfolio.storage.local.path = originalPath;
    });

    test('should validate S3 configuration when selected', async () => {
      // Test with incomplete S3 config
      config.portfolio.storage.type = 's3';
      config.portfolio.storage.s3 = {
        bucket: 'test-bucket'
  ...
  describe('Image Processing Pipeline Tests with Various Formats', () => {
    ...
    test('should optimize images for web performance', async () => {
      ...
      // Processed images should be available
      expect(response.body.data.image_url).toBeTruthy();
    });
  });

  describe('Portfolio Limits and Quota Enforcement Tests', () => {
    test('should enforce maximum image count limit', async () => {
      ...
      expect(portfolioResponse.body.data.portfolio).toHaveLength(2);
    });
    ...
    test('should enforce allowed file types', async () => {
      ...
      const response = await request(app)
        .post('/api/users/profile/portfolio')
        .set('Authorization', `Bearer ${providerToken}`)
        .attach('image', pngImage, 'type-limit-test.png');
    ...
    test('should provide accurate portfolio statistics', async () => {
      ...
      await request(app)
        .post('/api/users/profile/portfolio')
        .set('Authorization', `Bearer ${providerToken}`)
        .attach('image', testImage1, 'stats-test-1.jpg');

      const testImage2 = await createTestImage(400, 300, 'jpeg');
      await request(app)
        .post('/api/users/profile/portfolio')
        .set('Authorization', `Bearer ${providerToken}`)
        .attach('image', testImage2, 'stats-test-2.jpg');
      ...
      expect(statsResponse.status).toBe(200);
      expect(statsResponse.body.data.totalImages).toBe(2);
      expect(statsResponse.body.data.maxImages).toBe(5);
      expect(statsResponse.body.data.remainingSlots).toBe(3);
      expect(statsResponse.body.data.maxFileSize).toBe(config.portfolio.limits.maxFileSize);
      expect(statsResponse.body.data.allowedTypes).toEqual(config.portfolio.limits.allowedTypes);
    });
  });
